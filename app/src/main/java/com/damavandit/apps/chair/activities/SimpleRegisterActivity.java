...
public class SimpleRegisterActivity extends AppCompatActivity implements ServiceConnection {

    private Button mButtonGetCode;
    private TextInputEditText mTextPhone;

    private AppMainService mMainService;
    private SimpleRegisterBroadcastReceiver mBroadcastReceiver;
    ...
    public class SimpleRegisterBroadcastReceiver extends BroadcastReceiver {

        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent != null && !TextUtils.isEmpty(intent.getAction())) {
                if (intent.getAction().equals(USERNAME_IS_EXIST_SENT_WITH_CODE)) {
                    Intent intent1 = new Intent(SimpleRegisterActivity.this, SmsVerificationActivity.class);
